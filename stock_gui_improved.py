...
class ModernStockGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("沪深A股数据查询系统 - 专业版")
    ...
    def setup_styles(self):
        ...
        style.configure('TButton', background=colors['button_bg'], foreground=colors['button_fg'], 
                       font=('Microsoft YaHei', 9), borderwidth=0, focuscolor='none')
        style.map('TButton',
    ...
    def create_widgets(self):
        ...
        # 创建查询区域
        self.create_query_area(left_frame)
    ...
    def create_toolbar(self, parent):
        ...
        # 测试按钮
        test_btn = ttk.Button(toolbar_frame, text="🧪 测试", command=self.test_function)
        ...
        ttk.Radiobutton(toolbar_frame, text="表格视图", variable=self.view_mode, 
                       value="table", command=self.switch_view).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Radiobutton(toolbar_frame, text="图表视图", variable=self.view_mode, 
                       value="chart", command=self.switch_view).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Radiobutton(toolbar_frame, text="双视图", variable=self.view_mode, 
                       value="both", command=self.switch_view).pack(side=tk.LEFT)
    ...
    def create_query_area(self, parent):
        ...
        stock_frame = ttk.Frame(query_frame)
        stock_frame.grid(row=row, column=1, sticky=tk.W, pady=(0, 5), padx=(5, 10))
        
        stock_entry = ttk.Entry(stock_frame, textvariable=self.stock_code_var, width=15)
        stock_entry.pack(side=tk.LEFT)
        
        # 股票代码快速选择按钮
        ttk.Button(stock_frame, text="🔍", command=self.show_stock_selector, 
    ...
    def create_chart_area(self, parent):
        """创建图表区域"""
        chart_frame = ttk.LabelFrame(parent, text="📈 K线图表", padding="10")
        ...
        chart_combo = ttk.Combobox(control_frame, textvariable=self.chart_type,
                                 values=["candlestick", "line", "ohlc", "volume"],
                                 state="readonly", width=12)
        chart_combo.pack(side=tk.LEFT, padx=(5, 10))
        chart_combo.bind("<<ComboboxSelected>>", lambda e: self.update_chart())
        
        # 移动平均线
        ttk.Label(control_frame, text="均线:").pack(side=tk.LEFT)
        self.ma_periods = tk.StringVar(value="5,10,20")
        ma_entry = ttk.Entry(control_frame, textvariable=self.ma_periods, width=10)
        ma_entry.pack(side=tk.LEFT, padx=(5, 10))
        
        ttk.Button(control_frame, text="更新图表", command=self.update_chart).pack(side=tk.LEFT)
        ...
        try:
            ...
            if chart_type == "candlestick":
                ...
                for i, (date, row) in enumerate(self.current_data.iterrows()):
                    ...
                    # 实体
                    self.ax.bar(i, abs(row['收盘'] - row['开盘']), 
                              bottom=min(row['收盘'], row['开盘']), 
                              color=color, width=0.6)
                    # 影线
                    self.ax.plot([i, i], [row['最低'], row['最高']], 
                               color=color, linewidth=1)
                
                # 设置x轴
                self.ax.set_xticks(range(0, len(dates), max(1, len(dates)//10)))
                self.ax.set_xticklabels([d.strftime("%m-%d") for d in dates[::max(1, len(dates)//10)]])
                
            elif chart_type == "line":
                # 折线图
                self.ax.plot(dates, self.current_data['收盘'], 'b-', linewidth=2)
                ...
                for i, (date, row) in enumerate(self.current_data.iterrows()):
                    color = 'red' if row['收盘'] >= row['开盘'] else 'green'
                    # 垂直线
                    self.ax.plot([i, i], [row['最低'], row['最高']], color=color, linewidth=1)
                    # 开盘价横线
                    self.ax.plot([i-0.3, i], [row['开盘'], row['开盘']], color=color, linewidth=2)
                    # 收盘价横线
                    self.ax.plot([i, i+0.3], [row['收盘'], row['收盘']], color=color, linewidth=2)
                ...
                colors = ['red' if close >= open else 'green' 
                         for close, open in zip(self.current_data['收盘'], self.current_data['开盘'])]
                self.ax.bar(range(len(dates)), self.current_data['成交量'], color=colors)
                self.ax.set_xticks(range(0, len(dates), max(1, len(dates)//10)))
                self.ax.set_xticklabels([d.strftime("%m-%d") for d in dates[::max(1, len(dates)//10)]])
                self.ax.set_ylabel('成交量')
            
            # 添加移动平均线
            try:
                ma_periods = [int(x.strip()) for x in self.ma_periods.get().split(',') if x.strip().isdigit()]
                for period in ma_periods:
                    if period < len(self.current_data):
                        ma = self.current_data['收盘'].rolling(window=period).mean()
                        if chart_type in ["line", "candlestick", "ohlc"]:
                            if chart_type == "line":
                                self.ax.plot(dates, ma, label=f'MA{period}', linewidth=1)
                            else:
                                self.ax.plot(range(len(dates)), ma, label=f'MA{period}', linewidth=1)
            except:
                pass
            
            self.ax.set_title(f"{self.current_stock} - {chart_type.upper()}")
            self.ax.legend()
    ...
    def switch_view(self):
        ...
        # 这里可以根据需要实现视图切换逻辑
        self.show_status(f"切换到{view_mode}视图", "info")
    
    def show_stock_selector(self):
        """显示股票选择器"""
        # 这里可以实现股票选择对话框
        messagebox.showinfo("提示", "股票选择器功能待实现")
